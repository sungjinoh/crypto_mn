...
class AssetPositionTracker:
    ...
    def check_pending_orders_for_conflicts(self, pending_orders: List[PendingOrder]) -> Dict:
        """
        ...
        for asset, net_position in projected_positions.items():
            ...
            total_gross_exposure = sum(abs(order.quantity) for order in contributing_orders)
            net_exposure = abs(net_position)
            
            # Check if significant netting is possible
            netting_ratio = 1 - (net_exposure / total_gross_exposure) if total_gross_exposure > 0 else 0
            ...
            if netting_ratio > 0.1:  # More than 10% netting possible
                netting_opportunities[asset] = {
                    ...
                    'severity': 'high_netting' if netting_ratio > 0.7 else 'medium_netting'
                }
    ...
    def generate_optimized_execution_plan(self, pending_orders: List[PendingOrder]) -> Dict:
        """
            ...
            for asset, order in pair_assets.items():
                ...
                if abs(allocated_qty) > abs(order.quantity) * 0.01:
                    pair_net_orders.append({
                        ...
                        'netting_ratio': 1 - (abs(allocated_qty) / abs(order.quantity)) if order.quantity != 0 else 0,
                        'pair_key': pair_key,
        ...
        optimized_total_exposure = sum(abs(order['net_quantity']) for order in optimized_orders)
        ...
        return {
            ...
            'savings_ratio': capital_savings / original_total_exposure if original_total_exposure > 0 else 0,
    ...
    def get_asset_summary(self) -> Dict:
        ...
        for asset, position in self.asset_positions.items():
            summary[asset] = {
                ...
                'direction': 'LONG' if position.net_quantity > 0 else 'SHORT' if position.net_quantity < 0 else 'NEUTRAL',
    ...
    def load_state(self, filepath: str = "state/asset_positions.json"):
        ...
        try:
            if os.path.exists(filepath):
                ...
                print(f"Loaded {len(self.asset_positions)} asset positions from {filepath}")
            else:
