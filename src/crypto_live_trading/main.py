...
def main():
    ...
    asset_tracker = AssetPositionTracker(conflict_threshold=0.1)  # 10% threshold for conflicts
    ...
    print(f"   💰 Capital savings: ${execution_plan['capital_savings']:.2f} ({execution_plan['savings_ratio']:.1%})")
    ...
    for pair_key, plan in execution_plan['execution_plan'].items():
        if plan['status'] == 'execute_netted':
            ...
            for order in plan['orders']:
                ...
                netting_info = f"({order['netting_ratio']:.1%} netted)" if order['netting_ratio'] > 0.1 else ""
                print(f"      • {order['asset']}: {direction} {abs(order['net_quantity']):.4f} {netting_info}")
        else:
    ...
    for pair_key, plan in execution_plan['execution_plan'].items():
        ...
        signal_info = next((si for si in pair_signals if si['pair_key'] == pair_key), None)
        if not signal_info:
        ...
        try:
            ...
            if signal.is_entry and not position:
                ...
                if result:
                    ...
                    tracker.record_open(symbol1, symbol2, net_signal, execution_result=result)
                    positions_opened += 1
                    ...
                    try:
                        ...
                        slack.notify_position_open(symbol1, symbol2, net_signal, position_info)
                    except Exception as e:
                        print(f"   Warning: Slack notification failed: {e}")

            elif signal.is_exit and position:
                print(f"   🔴 Closing position (was {position.side}) - using actual executed quantities")
                ...
                if result:
                    ...
                    tracker.record_close(symbol1, symbol2, signal, execution_result=result)
                    positions_closed += 1

                    # Send Slack notification for position close
                    try:
                        position_info = {"duration": duration_str}
                        slack.notify_position_close(symbol1, symbol2, net_signal, position_info)
                    except Exception as e:
                        print(f"   Warning: Slack notification failed: {e}")
    ...
    print(f"\n🔍 STEP 4: Monitoring existing positions...")
    for pair_key, position in open_positions.items():
        if position.status == "open":
            ...
            print(f"   ⏸️  Holding {position.side} position (opened: {position.entry_time[:19]})")

            # Monitor margin levels for existing position
            try:
                margin_status = executor.monitor_position_margin(symbol1, symbol2, slack)
                    ...
                    for symbol_key in ["symbol1", "symbol2"]:
                        if symbol_key in margin_status:
                            ...
                            if status.get("action") != "none":
                                print(f"      {status.get('message', 'Margin action taken')}")
                else:
                ...
                try:
                    slack.notify_error(f"Margin monitoring failed: {e}", symbol1, symbol2)
                except:
                    pass
    ...
    executed_pairs = sum(1 for plan in execution_plan['execution_plan'].values() if plan['status'] == 'execute_netted')
    netted_out_pairs = sum(1 for plan in execution_plan['execution_plan'].values() if plan['status'] == 'skip_netted_out')
    ...
    print(f"💡 Position netting applied: {'Yes' if netting_analysis.get('has_netting_opportunities') else 'No'}")
    ...
    if updated_positions:
        ...
        for pair_key, pos in updated_positions.items():
            print(f"  • {pos.symbol1}-{pos.symbol2}: {pos.side} (opened: {pos.entry_time[:19]})")
            
    if asset_summary:
        print(f"\nAsset-Level Net Positions:")
        for asset, info in asset_summary.items():
            if info['net_quantity'] != 0:
                print(f"  • {asset}: {info['direction']} {abs(info['net_quantity']):.6f} (from {info['pairs_count']} pairs)")
    
    # Show final net positions from execution plan  
    final_net_positions = {k: v for k, v in execution_plan['net_positions'].items() if abs(v) > 0.001}
